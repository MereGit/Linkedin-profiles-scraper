...
def is_correct_role_ai(linkedin_url: str, expected_role: str, firm: str) -> Tuple[bool, str, str, str, str]:
	...
	logger.info(f"DuckDuckGo lookup for: {linkedin_url}")
	...
	for result in results:
		...
		if linkedin_validator.is_linkedin_profile_url(link) == True:
			...
			agent = initialize_agent([], llm_ag, agent="conversational-react-description", verbose = False)
			...
			if check == "TRUE":
				...
				logger.info(f"Validation result: Wrong role, matching firm (status={check})")
				return True, "Wrong role, matching firm", link, check, title
			elif check == "DIFFERENT_FIRM":
				logger.info(f"Validation result: Matching role, different firm (status={check})")
				return True, "Matching role, different firm", link, check, title

	logger.warning(f"No LinkedIn profile found in DuckDuckGo results for {linkedin_url}")
